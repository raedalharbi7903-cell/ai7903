---
title: Degraded Analysis Policy
document_id: DOC-DEGRADED-001
version: 1.0.0
status: proposed
owner: Engineering
reviewers: CTO
...
# Degraded Analysis Policy
## Purpose
Define failure transparency.
## Scope
Architecture and governance specification only; no runtime behavior is implemented by this document.
## Main Content
Provider failure, stale data, missing timeframes, engine failure, or incomplete evidence must yield explicit failure, partial/degraded output, or no-decision. Exact threshold policy is Decision Required.
